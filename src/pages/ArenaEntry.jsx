...
const SmartTextField = ({ label, value, onChange, placeholder, isShort }) => {
    ...
    return (
        <Box sx={{ position: 'relative', mb: 2, width: '100%' }}>
            <TextField fullWidth multiline={!isShort} rows={isShort ? 1 : 3} label={label} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} 
                InputProps={{
                    endAdornment: (
...
function ArenaEntry() {
    ...
    const handleScanSuccess = (result) => {
        if (result && result.length > 0) {
            const match = result[0].rawValue.match(/play\/([A-Z0-9]+)/i);
            if (match && match[1]) { setOpenScanner(false); navigate(`/arena/play/${match[1]}`); } 
            else if (result[0].rawValue.length <= 6) { setOpenScanner(false); navigate(`/arena/play/${result[0].rawValue}`); } 
    ...
    const handleSaveCustomQuestion = async () => {
        ...
        const payload = {
            ...
            content: customQ.content, question_type: customQ.question_type, explanation: customQ.explanation, short_answer: customQ.shortAnswer,
        ...
        } catch (err) { showToast('Lỗi khi lưu câu hỏi.'); }
    };
                    ...
                    {customQ.question_type === 'TF' && (
                        <Box>
                            {customQ.optionsTF.map((opt, idx) => (
                                <Box key={idx} display="flex" alignItems="flex-start" gap={2} mb={1}>
                                    <Select size="small" value={opt.is_correct} onChange={(e) => { const newOpts = [...customQ.optionsTF]; newOpts[idx].is_correct = e.target.value; setCustomQ({...customQ, optionsTF: newOpts}); }} sx={{ mt: 1, minWidth: 100 }}>
